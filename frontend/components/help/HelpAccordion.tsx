  ...
  return (
    <div className="grid gap-3">
      {modules.map((module, index) => (
        <RevealOnScroll key={module.key} delayMs={(index % 5) * 70}>
          <details
            className="public-motion-card group rounded-xl border border-border bg-card text-card-foreground shadow-card"
            open={index === 0}
          >
